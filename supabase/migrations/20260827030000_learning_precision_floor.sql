-- Keep exposure-aware decay observable at the stored numeric(4,3) precision.
-- This is a storage precision floor, not a larger learning allowance: every
-- update remains bounded and the raw signal continues to shrink with exposure.
...
begin
  ...
  delta := sign(raw_delta) * greatest(abs(raw_delta), 0.001);
  ...
  insert into public.audit_logs (
    ...
    jsonb_build_object(
      ...
      'signal', signal_value,
      'prior_category_signals', prior_signal_count,
      'raw_delta', raw_delta,
      'applied_delta', delta
    )
  );
